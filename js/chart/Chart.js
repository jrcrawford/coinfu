...
window.Chart = function(context){
	...
	if (window.devicePixelRatio) {
		context.canvas.style.width = "100%";//width + "px";
	...
	var Line = function(data,config,ctx){
		...
		function drawScale(){
			...
			while (i<data.labels.length){
                i=i+Math.max(Math.floor(data.labels.length/15),1);
    ...
    function drawLegend(i, yAxisPosX, spacer, data, ctx) {
        ...
        // Draw legend
        xRect = x + spacer.gap + space + 2;
        yRect = spacer.legendY-sqrSize/2;
        ctx.rect(xRect, yRect, sqrSize, sqrSize);
