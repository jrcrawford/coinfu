...
function RagChart(id, data, labels, options){
	...
	this.renderFigs = function(){
		...
		for(i=0; i<rgaLength; i++){
            if (i===rgaLength-1) rd[i] = rt;

			if(opts && opts.customColor){
				html += '<li'+rgaLi+'><div class="cf-figs-fig"><p style="color: '+opts.customColor[i]+';">'+rd[i].toFixed(6)+''+postfix+'</p></div></li>';
			}
			else{
				if(rga[i] == undefined){
					var rgai = 'm-missing';
				...
				html += '<li'+rgaLi+'><div class="cf-figs-fig '+rgai+'"><p>'+rd[i].toFixed(6)+''+postfix+'</p></div></li>';
			}
		}
		html += '</ul></div>';
...
*/function FunnelChart(id, data, labels, options){
	...
	this.renderFunnel = function(){
		...
		for(var i=0; i<fll; i++){
			...
			if(opts && opts.customColors){
				html += '<li style="'+height+' width:'+fdp[i]+'%; opacity:'+os+'; background-color: '+opts.customColors[i]+'"></li>';
			}
			else{
				html += '<li class="m-green mb" style="'+height+' width:'+fdp[i]+'%; opacity:'+os+'"></li>';
			}
		}
	...
	this.renderFigs = function(){
		...
		for(var i=0; i<fll; i++){
			...
			if(opts && opts.customColors){
				html += '<li style="'+height+'"><div><p class="metric" style="opacity:'+os+'; color:'+opts.customColors[i]+'">'+fd[i]+'</p></div><div><a href="#/minerstats/' + fl[i] + '">'+fl[i]+'</a></div></li>';
			}
			else{
				html += '<li style="'+height+'"><div><p class="metric m-green mc" style="opacity:'+os+'">'+fd[i]+'</p><a href="#/minerstats/' + fl[i] + '">'+fl[i]+'</a></div></li>';
			}
		}
